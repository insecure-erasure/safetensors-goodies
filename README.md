...
Inside a full checkpoint, every tensor carries a key that reflects its position within the complete model hierarchy. A UNet weight in an SDXL checkpoint, for example, is stored as `model.diffusion_model.input_blocks.0.0.weight`. That prefix makes sense when everything lives in the same file, but once the UNet is extracted into its own `.safetensors` file, ComfyUI expects to load it as a standalone model — and its loader looks for `input_blocks.0.0.weight`, without the enclosing path.

The script strips these container prefixes automatically on extraction so the output file is directly usable in ComfyUI without any manual post-processing. The transformation is always a pure prefix removal: no keys are renamed, reordered, or otherwise modified — only the leading path segment that identifies the component's location within the original checkpoint is dropped.

To make this concrete, an SDXL checkpoint stores tensors from all four components in a single flat dictionary. A small sample:

```
# Full checkpoint (all components share the same flat namespace)
model.diffusion_model.input_blocks.0.0.weight        # UNet
model.diffusion_model.input_blocks.0.0.bias          # UNet
model.diffusion_model.output_blocks.0.0.weight       # UNet
first_stage_model.encoder.conv_in.weight             # VAE
first_stage_model.decoder.conv_out.weight            # VAE
conditioner.embedders.0.transformer.text_model.embeddings.position_ids   # CLIP-L
conditioner.embedders.0.transformer.text_model.encoder.layers.0.weight   # CLIP-L
conditioner.embedders.1.model.transformer.resblocks.0.attn.in_proj.weight # CLIP-G
conditioner.embedders.1.model.ln_final.weight                             # CLIP-G
```

After extraction, each component lives in its own file with the container prefix removed:
...
# clip_g.safetensors
transformer.resblocks.0.attn.in_proj.weight
ln_final.weight
```

If you need the original key names — for example to inspect the raw structure, diff against another checkpoint, or load the file with a tool that expects the full paths — pass `--keep-original-keys`. The extracted file will be identical in content but with keys as they appear in the source checkpoint.
...
| `--text-encoder-2-precision` | Precision override for second text encoder |# safetensors-extract
